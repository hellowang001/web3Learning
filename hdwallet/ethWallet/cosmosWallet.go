...
func main() {
	...
	sha256Hash := sha256Hasher.Sum(nil)     // 这一步是在干嘛？

	// 然后对 SHA-256 哈希结果进行 RIPEMD-160 哈希加密处理
	ripemd160Hasher := ripemd160.New()     //  同样 创建ripemd-160这个对象
	ripemd160Hasher.Write(sha256Hash)      // 这里是在对之前sha256加密后的公钥再进行RIPEMD-160 加密
	pubKeyHash := ripemd160Hasher.Sum(nil) // 这一步是在干嘛？
